...
def make_env(rank, env_conf, seed=0):
    def _init():
        env = StreamWrapper(
            PokeRedEnv(
                gb_path=env_conf['gb_path'],
                init_state=env_conf['init_state'],
                max_steps=env_conf['max_steps'],
                headless=env_conf['headless'],
                action_frequency=env_conf['action_freq']
            ),
            stream_metadata={
                "user": "v3-test",
                "env_id": rank,
...
if __name__ == '__main__':
    ...
    learn_steps = 40
    file_name = 'session_tomsession/poke_38207488_steps' #'session_e41c9eff/poke_250871808_steps'
    ...
    if exists(file_name + '.zip'):
        ...
        model = PPO('MultiInputPolicy', env, verbose=1, n_steps=ep_length, batch_size=16, n_epochs=1, gamma=0.999)
